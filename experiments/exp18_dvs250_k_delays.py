...
tau_m = 'normal'
delay = (250,1)
ckpt_dir = 'exp17_shd250_pruned_k' 

snn = P_DelaySNN(dataset_dict=dataset_dict, structure=(64, 2), connection_type='f',
    delay=delay, delay_type='h', n_pruned_delays=250, tau_m = tau_m,
    win=total_time, loss_fn='mem_sum', batch_size=batch_size, device=device,
    debug=False)

snn.model_name = 'DynamicGradualPruningSHD250_' + snn.model_name
...
for n_delays in range(250,0, -1):
    snn.f1_f2.top_k = n_delays
    print("pruned to: " + str(snn.f1_f2.top_k))
    train(snn, train_loader, test_loader, lr, 10, dropout=0.0, 
        test_behavior=tb_save_max_last_acc, ckpt_dir=ckpt_dir, scheduler=(1000, 0.95), test_every=1)