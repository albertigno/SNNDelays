...
time_window = 100
batch_size = 64
num_epochs = 5000 # iterations steps
...
#### first run (f+d)
sweep_params = {
    'delay_range': [(x, 1) for x in range(5, 105, 5)],
    'structure':[(8, 2, 'd'), (64, 2, 'd')]
    }

sweep_params_names = {
    'delay_range': ['drf'+str(x) for x in range(5, 105, 5)],
    'structure':['h8', 'h64']
    }
...
def train_model(cfg_id, repetition):
    ...
    snn.save_model(snn.initial_model_nam, ckpt_dir)
