...
time_window = 20
batch_size = 64
num_epochs = 1000 # iterations steps
...
train_params = {'learning_rate':1e-3, 'num_epochs':1, 'scheduler':(100, 0.95), 'ckpt_dir':ckpt_dir,
          'test_behavior':tb_synthetic_refact, 'test_every':100, 'printed_steps':10, 'freeze_taus':True}

#### first run (f+d)
sweep_params = {
    'delay_range': [(x, 1) for x in range(5, 15, 5)],
    'structure':[(8, 2, 'd'), (64, 2, 'd')]
    }

sweep_params_names = {
    'delay_range': ['drf'+str(x) for x in range(5, 15, 5)],
    'structure':['h8', 'h64']
    }
