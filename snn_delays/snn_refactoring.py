...
CHANGES: retain_graph = True in loss.backward()
...
class AbstractSNNLayer:
    ...
    def activation_function(self, mem, thresh, th_reset):

        '''
        The activation function is defined here
        '''

        #self.th_reset = thresh
...
class FeedforwardSNNLayer(nn.Module, AbstractSNNLayer):
    ...
    def __init__(self, num_in, num_out, tau_m, batch_size, inf_th = False, device = None, delays = None):
        '''
        ...
        self.pre_delays = delays is not None
        #self.step = step
        #self.max_d = max_d
        self.device = device
        self.thresh = 0.3
        ...
        if self.pre_delays:
            ...
            self.register_buffer("delayed_spike_queue", torch.zeros(
                    self.batch_size, max_d+1, num_in, device=self.device)) # complete with appropriate values
    ...
    def forward(self, prev_spikes):
        ...
        if self.pre_delays:
            ...
            self.update_queue(self.delayed_spike_queue, prev_spikes)
            self.spikes = self.delayed_spike_queue[:, self.delays, :].transpose(1, 2)

        # propagate previous spikes (wheter on simple or extended form)
        self.mems, self.spikes = self.update_mem(
            prev_spikes.reshape(self.batch_size, -1), self.spikes, self.mems.clone(), self.thresh)
