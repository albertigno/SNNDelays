...
CHANGES: 
v2: layer mems, spikes defined externally. retain graph and clone, detach no needed
'''
...
class FeedforwardSNNLayer(nn.Module, AbstractSNNLayer):
    ...
    def forward(self, prev_spikes, own_mems, own_spikes):
        ...
        if self.pre_delays:
            ...
            prev_spikes = self.delayed_spike_queue[:, self.delays, :].transpose(1, 2).clone().detach() # is transpose necessary?
            #prev_spikes = self.delayed_spike_queue[:, self.delays, :].clone().detach()
            # print(f'plus_delayed_spikes: {prev_spikes.shape}')
