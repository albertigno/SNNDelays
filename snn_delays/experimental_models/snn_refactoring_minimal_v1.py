...
- self mems and spikes detach().clone() before mem_update

v1: layer mems, spikes defined in self (FeedForwardSNNLayer) 
'''
...
class FeedforwardSNNLayer(nn.Module, AbstractSNNLayer):
    ...
    def __init__(self, num_in, num_out, tau_m, batch_size, inf_th = False, device = None, fanin_delays = None):
        ...
        self.spikes = torch.zeros(
                batch_size, num_out, device=device)
    ...
    def forward(self, prev_spikes):
        ...
        if self.pre_delays:
            '''
            update queue
            get spikes to be processed by the next layer from queue 
            '''
            #print(f'arriving_spikes: {torch.sum(prev_spikes)}')
            # print(f'plus_delayed_spikes: {prev_spikes.shape}')
            # print(f'arriving_spikes: {prev_spikes[0, 0:5]}')
            self.delayed_spike_queue = self.update_queue(self.delayed_spike_queue, prev_spikes)
            #prev_spikes = self.delayed_spike_queue[:, self.delays, :].transpose(1, 2).clone() # is transpose necessary?
            prev_spikes = self.delayed_spike_queue[:, self.delays, :].clone()
            # print(f'plus_delayed_spikes: {prev_spikes.shape}')
            # print(f'plus_delayed_spikes: {prev_spikes[0, :, 0:5]}')
            #print(f'plus_delayed_spikes: {torch.sum(prev_spikes)}')

        spikes_local = self.spikes.clone().detach()
        mems_local = self.mems.clone().detach()
        ...
        mems, spikes = self.update_mem(
            prev_spikes.reshape(self.batch_size, -1), spikes_local, mems_local, self.thresh)
...
class SNN(Training, nn.Module):
    ...
    def forward(self, input):
        ...
        for step in range(self.win):
            ...
            # last layer
            all_o_mems.append(self.layers[-1].mems)
            all_o_spikes.append(self.layers[-1].spikes)
